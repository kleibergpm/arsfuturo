...
r.post('/auth/login', validate(authBody), asyncHandler(authController.login)); r.get('/auth/me', authenticate, asyncHandler(authController.me));
...
r.get('/autorizaciones', authenticate, allow(...roles.staff), asyncHandler(crud('autorizacion', { afiliado: true, proveedor: true }).list)); r.get('/autorizaciones/:id', authenticate, allow(...roles.staff), asyncHandler(crud('autorizacion', { afiliado: true, proveedor: true }).get)); r.post('/autorizaciones', authenticate, allow(...roles.staff), validate(schemas.autorizacion), asyncHandler(businessController.createAuthorization)); r.patch('/autorizaciones/:id/aprobar', authenticate, allow(...roles.review), asyncHandler(businessController.decideAuthorization('APROBADA'))); r.patch('/autorizaciones/:id/rechazar', authenticate, allow(...roles.review), asyncHandler(businessController.decideAuthorization('RECHAZADA')));
r.get('/servicios', authenticate, allow(...roles.staff), asyncHandler(crud('servicioMedico', { afiliado: true, proveedor: true, autorizacion: true }).list)); r.post('/servicios', authenticate, allow(...roles.staff), validate(schemas.servicio), asyncHandler(businessController.createService));
r.get('/pagos', authenticate, allow(...roles.admin), asyncHandler(crud('pagoProveedor', { servicio: true, proveedor: true }).list)); r.post('/pagos', authenticate, allow(...roles.admin), validate(schemas.pago), asyncHandler(businessController.issuePayment));
r.get('/facturas', authenticate, allow(...roles.admin), asyncHandler(crud('factura', { poliza: true }).list)); r.post('/facturas/generar', authenticate, allow(...roles.admin), validate(z.object({ periodo: z.string().regex(/^\d{4}-\d{2}$/).optional() })), asyncHandler(businessController.generateInvoices)); r.patch('/facturas/:id/pagar', authenticate, allow(...roles.admin), validate(z.object({ referencia: z.string().min(3) })), asyncHandler(businessController.payInvoice)); r.patch('/facturas/:id/recordatorio', authenticate, allow(...roles.admin), asyncHandler(businessController.remindInvoice)); r.patch('/facturas/:id/gracia', authenticate, allow(...roles.admin), asyncHandler(businessController.graceInvoice)); r.patch('/facturas/:id/suspender', authenticate, allow(...roles.admin), asyncHandler(businessController.suspendInvoice));
r.get('/dashboard/resumen', authenticate, asyncHandler(businessController.dashboard));
export default r;
